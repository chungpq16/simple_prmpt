"""
Core prompt generation logic based on the Metaprompt system.
Implements the concept from prompt_generator.ipynb.
"""
import re
...
class PromptGenerator:
    """Generates custom prompts based on user tasks using the Metaprompt system."""
    
    def __init__(self, llm_client: LLMFarmClient):
        """
        Initialize the prompt generator.
        
        Args:
            llm_client: LLM Farm client for API calls
        """
        self.llm_client = llm_client
        logger.info("PromptGenerator initialized")
    
    @property
    def metaprompt(self) -> str:
        """The complete metaprompt template from the notebook with all 6 examples."""
...
Note: If you want the AI to output its entire response or parts of its response inside certain tags, specify the name of these tags (e.g. "write your answer inside <answer> tags") but do not include closing tags or unnecessary open-and-close tag sections.'''
    ...
    def strip_last_sentence(self, text: str) -> str:
    ...
    def find_free_floating_variables(self, prompt: str) -> List[str]:
        """Find variables that are not properly enclosed in XML tags."""
    ...
    def generate_prompt_template(self, task: str, variables: Optional[List[str]] = None) -> Dict[str, any]:
        """
        ...
        """
        try:
            logger.info(f"Generating prompt template for task: {task[:50]}...")
            
            # Prepare the prompt for the metaprompt
            prompt = self.metaprompt.replace("{{TASK}}", task)
            
            # Create assistant partial based on variables
            ...
            result = {
                ...
                "task": task,
                "floating_variables": floating_vars
            }
            
            logger.info(f"Successfully generated prompt template with {len(extracted_variables)} variables")
            return result
    ...
    def test_prompt_template(self, prompt_template: str, variable_values: Dict[str, str]) -> str:
        ...
        Args:
            ...
            variable_values: Dictionary of variable names and their values
            
        Returns:
            The AI's response to the filled prompt template
        """
        try:
            ...
            for variable, value in variable_values.items():
                filled_prompt = filled_prompt.replace("{" + variable + "}", value)
            
            # Test with LLM
            response = self.llm_client.completion(filled_prompt)
            
            logger.info("Prompt template test completed successfully")
            return response
            
        except Exception as e:
            logger.error(f"Error testing prompt template: {str(e)}")
            raise Exception(f"Failed to test prompt template: {str(e)}")